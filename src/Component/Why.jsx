...
function Why() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (!card1Ref.current || !card2Ref.current || !card3Ref.current || !card4Ref.current) return;
      ...
      tl.to(card2Ref.current, { y: 0, opacity: 1, duration: 0.15 }, "card2-=0.05");
      tl.to(card1Ref.current, { scale: 0.95, filter: "blur(5px)", duration: 0.15 }, "card2");
      tl.to(card3Ref.current, { y: 0, opacity: 1, duration: 0.15 }, "card3-=0.05");
      tl.to(card2Ref.current, { scale: 0.95, filter: "blur(5px)", duration: 0.15 }, "card3");
      tl.to(card4Ref.current, { y: 0, opacity: 1, duration: 0.15 }, "card4-=0.05");
      tl.to(card3Ref.current, { scale: 0.95, filter: "blur(5px)", duration: 0.15 }, "card4");
    });
  ...
  return (
    <div className="">
      <div className="why-content h-screen w-full overflow-hidden" ref={sectionRef}>
        <div className="why-header h-fit w-full px-8 py-12 md:py-18 md:px-16 text-center" ref={headerRef}>
          <h3 className='text-2xl leading-none md:text-5xl lg:text-5xl pb-4 text-[#D9D9D9] font-[Roboto_Flex] font-black tracking-wide overflow-hidden px-4'>
            WHY TO CHOOSE AAKAAR?
          </h3>
          <span className='text-zinc-400 leading-none font-[Dancing_Script] font-extralight text-lg md:text-2xl lg:text-2xl   overflow-hidden px-4'>
            " Rooted in design. Focused on growth. Driven by impact. "
          </span>
        </div>

        <div className="why-cardsection flex items-center flex-col justify-center  h-[75vh] w-full px-4 md:px-5">
          <div className="cards-stack relative h-[70vh] w-full max-w-[900px]">
            {cardRefs.map((ref, i) => (
              <div
                key={i}
                ref={ref}
                className="card h-[70vh] w-full bg-[#27170e] rounded-lg flex flex-col md:flex-row absolute top-0 left-0 items-center justify-center gap-6 shadow-xl my-4 md:my-0 px-4 md:px-6 lg:px-12"
              >
                {/* Lazy loaded image */}
                <img
                ...
                />
                <div className="clarity-dets text-white h-full  flex flex-col md:flex-col  items-start absolute justify-start gap-8 md:gap-0 z-10 px-4">
                  <div className="s1">
                    <h5 className='font-[Tangerine] text-[#bababa] text-[7rem] md:text-[15rem]  lg:text-[13rem]  lg:pl-0  leading-none'>
                      {`0${i + 1}.`}
                    </h5>
                  </div>
                  <div className="s1-info text-start  md:pr-20 py-4 lg:pb-12">
                    <h5 className='text-5xl md:text-7xl text-white md:pt-20 lg:pt-6 lg:text-[3.9rem] font-bold font-[Dancing_Script] py-6'>
                      {titles[i]}
                    </h5>
                    <div className="">
                      {descriptions[i].map((para, index) => (
                        <p
                          key={index}
                          className='font-[Familjen_Grotesk] text-zinc-300 text-lg md:text-2xl pb-4 lg:text-[1.6rem]  leading-snug'
                        >
                          {para}
                        </p>
                      ))}
